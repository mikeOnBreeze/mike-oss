...
// of whichever provider the user has keys for. OpenRouter and Claude use Claude
// Haiku; Gemini-only setups use Gemini Flash Lite.
function resolveTitleModel(apiKeys: UserApiKeys): string {
    if (apiKeys.openrouter?.trim()) return DEFAULT_TITLE_MODEL;
...
export async function getUserModelSettings(
    ...
    const { data } = await client
        .from("user_profiles")
        .select(
            "tabular_model, claude_api_key, gemini_api_key, openrouter_api_key",
        )
        .eq("user_id", userId)
        .single();
